    ...
    def execute_launch(self):
        ...
        if args.use_internal_libs:
            if sys.platform == "win32":
                ...
                # Isaac Sim 5.1 ships the ROS libraries under the bridge
                # extension.  Newer layouts may use isaacsim.ros2.core, so
                # resolve the directory instead of hard-coding one release.
                ...
                internal_library_path = next(
                    (
                        path
                        for path in internal_library_candidates
                        if os.path.isdir(path)
                    ),
                    None,
                )
                if internal_library_path is None:
                    print(
                        "ERROR: Isaac Sim's internal ROS libraries were not "
                        f"found for ROS {args.ros_distro}. Tried: "
                        + ", ".join(internal_library_candidates),
                        file=sys.stderr,
                    )
                    sys.exit(1)
                ...
                os.environ["LD_LIBRARY_PATH"] = os.pathsep.join(
                    path
                    for path in [internal_library_path, current_library_path]
                    if path
                )
