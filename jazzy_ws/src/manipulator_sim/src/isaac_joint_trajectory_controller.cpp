...
public:
  ...
  : Node("isaac_joint_trajectory_controller", options)
  {
    ...
    if (path_tolerance_rad_ <= 0.0 || goal_tolerance_rad_ <= 0.0 ||
      goal_time_tolerance_s_ <= 0.0 || stable_samples_required_ <= 0)
    {
    ...
    state_subscription_ = create_subscription<sensor_msgs::msg::JointState>(
      ...
      std::bind(
        &IsaacJointTrajectoryController::joint_state_callback, this,
        std::placeholders::_1));
...
private:
  ...
  {
    // The six UR model joints are bounded revolute joints, not continuous
    // joints.  Wrapping the error modulo 2*pi can falsely treat positions near
    // opposite joint limits as equal.
    ...
    double & state_age_s) const
  {
    ...
    state_age_s = std::chrono::duration<double>(
      std::chrono::steady_clock::now() - latest_state_time_).count();
    positions.clear();
    ...
    std::string & reason) const
  {
    ...
    const auto tolerances_are_valid = [&trajectory](
    ...
    if (!tolerances_are_valid(goal.path_tolerance) ||
      !tolerances_are_valid(goal.goal_tolerance))
    {
    ...
    const std::shared_ptr<GoalHandleFollowJointTrajectory> goal_handle)
  {
    ...
    execution_thread_ = std::thread(
      &IsaacJointTrajectoryController::execute, this, goal_handle);
  }
    ...
    double elapsed_s) const
  {
    ...
    if (elapsed_s <= first_time) {
      ...
      for (std::size_t index = 0; index < output.size(); ++index) {
        output[index] = initial_positions[index] +
          alpha * (points.front().positions[index] - initial_positions[index]);
      }
      return output;
    }
    ...
    for (std::size_t index = 1; index < points.size(); ++index) {
      ...
      if (elapsed_s <= segment_end) {
        ...
        const double alpha = duration <= std::numeric_limits<double>::epsilon() ?
          1.0 : std::clamp((elapsed_s - segment_start) / duration, 0.0, 1.0);
        std::vector<double> output(initial_positions.size());
        for (std::size_t joint_index = 0; joint_index < output.size(); ++joint_index) {
          output[joint_index] = points[index - 1].positions[joint_index] +
            alpha * (
            points[index].positions[joint_index] -
            points[index - 1].positions[joint_index]);
        }
        return output;
      }
    }
  ...
  std::vector<double> tolerances_for(
  ...
  void execute(const std::shared_ptr<GoalHandleFollowJointTrajectory> goal_handle)
  {
    ...
    if (!read_positions(names, initial_positions, state_age_s) ||
      state_age_s > state_timeout_s_)
    {
    ...
    const auto path_tolerances = tolerances_for(
      goal->path_tolerance, path_tolerance_rad_, names);
    const auto goal_tolerances = tolerances_for(
      goal->goal_tolerance, goal_tolerance_rad_, names);
    const double requested_goal_time = duration_seconds(goal->goal_time_tolerance);
    const double goal_time_tolerance = requested_goal_time > 0.0 ?
      requested_goal_time : goal_time_tolerance_s_;
    const double final_trajectory_time = duration_seconds(
